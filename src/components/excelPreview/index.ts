...
/** 获取数据、基本切换 */
export function usePrevirewExcel() {
  /** Excel基础数据 */
  ...
  const getExcelData = async (link: string = './test.xlsx'): Promise<boolean> => {
    ...
    return new Promise((resolve, reject) => {
      axi({
        ...
        const workbook: WorkBook = XLSX.read(new Uint8Array(data as ArrayBuffer), {
          type: 'array',
          cellStyles: true,
        })
  ...
  const sheetExcelTab = async (index: number = 0): Promise<boolean> => {
    return new Promise(async (resolve, reject) => {
      ...
      const worksheet: WorkSheet = JSON.parse(JSON.stringify(table_state.info_data[table_state.table_sheet[index]]))
      if (worksheet.A1) {
        // 渲染某一个sheet数据
...
/** sheet 切换位移相关 */
export function sheetTabScorllAbout() {
  ...
  const addSlotHtml = (): Promise<boolean> => {
    return new Promise((resolve, reject) => {
      // 选择渲染出来的所有 td 单项
      const excel_td_dom: NodeListOf<any> = document.querySelectorAll('#excel_body_id table td')
      for (const i in excel_td_dom) {
        // 内容为空高度丢失问题
